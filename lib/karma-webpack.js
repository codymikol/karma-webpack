...
function preprocessorFactory(config, emitter) {
  ...
  const transformPath = (config.webpack && config.webpack.transformPath) || defaultTransformPath;
   ...
   * @returns {Promise<any>}
   */
  const processFile = async (_content, file, done) => {
