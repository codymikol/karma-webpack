...
/** @typedef {import('karma').Server} KarmaServer */

class KarmaSyncPlugin {
  /**
   * @param {{ karmaEmitter: KarmaServer, controller: KarmaWebpackController }} options
   */
  constructor(options) {
  ...
  apply(compiler) {
    ...
    compiler.hooks.done.tap('KarmaSyncPlugin', async (stats) => {
      ...
      stats.toJson().assets.forEach((/** @type {any} */ webpackFileObj) => {
        let outputPath = path.join(os.tmpdir(), '_karma_webpack_');
        if (
          compiler.options &&
        ...
        const filePath = `${outputPath}/${webpackFileObj.name}`;
...
const defaultOutputPath = path.join(os.tmpdir(), '_karma_webpack_');
...
class KarmaWebpackController {
  ...
  /**
   * @param {KarmaServer | null} emitter
   */
  set karmaEmitter(emitter) {
    if (emitter) {
      this.__karmaEmitter = emitter;

      if (!this.webpackOptions.plugins) 
        this.webpackOptions.plugins = [];
 
      this.webpackOptions.plugins.push(
        new KarmaSyncPlugin({
  ...
  /**
   * @returns {KarmaServer | null} emitter
   */
  get karmaEmitter() {
  ...
  get outputPath() {
    if (!this.webpackOptions.output || !this.webpackOptions.output.path) 
  ...
  constructor() {
    ...
    /** @type {KarmaServer | null} */
    this.__karmaEmitter = null;

    /** @type {webpack.Compiler | null} */
    this.compiler = null;

    /** @type {boolean | NodeJS.Timer} */
  ...
  /**
   * @param {webpack.Configuration} newOptions
   */
