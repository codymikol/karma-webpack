...
Plugin.prototype.make = function (compilation, callback) {
  async.forEach(this.files.slice(), (file, callback) => {
    ...
    compilation.addEntry('', dep, path.relative(this.basePath, file)
      .replace(/\\/g, '/'), (err) => {
...
Plugin.prototype.readFile = function (file, callback) {
  ...
  const doRead = function () {
    if (optionsCount > 1) {
      ...
      try {
        const fileContents = middleware.fileSystem.readFileSync(path.join(os.tmpdir(), '_karma_webpack_', file.replace(/\\/g, '/')));
...
function createPreprocesor(/* config.basePath */ basePath, webpackPlugin) {
  return function (content, file, done) {
    ...
    webpackPlugin.readFile(path.relative(basePath, file.originalPath), (err, content) => {
      if (err) {
        throw err;
      }
